"""
Tests pour le solveur CBR et les Sémantiques d'Argumentation.

Conventions :
- Chaque test est une fonction indépendante
- On construit les AF manuellement (pas de Parser)
- Un résumé final indique les tests passés / échoués
"""

import sys
sys.path.insert(0, '.')  # adapter selon la structure du projet
...
from Src.Solver.Specialist.TransformationSpecialist.ArgumentTransformationSpecialist import ArgumentTransformationSpecialist
from Src.Solver.Specialist.TransformationSpecialist.AttackTransformationSpecialist import AttackTransformationSpecialist
from Src.Solver.Strategy.TransformAndIsomorphStrategy import TransformAndIsomorphStrategy
...
def makeAF(*args, attacks=[]):
    """Crée un AF avec les indices donnés et les attaques (from, to)."""
    af = ArgFramework()
    for i in args:
...
af_b = makeAF(3, 4, attacks=[(3, 4)])  # isomorphe de af_a
...
# Utilisation de graphes à 3 nœuds pour valider la directionnalité
...
# ─────────────────────────────────────────────
# 3. IsomorphismSpecialist
# ─────────────────────────────────────────────
print("\n── IsomorphismSpecialist ──")
...
test("iso direct : réponse correcte (True)",   sol is not None and sol.getAnswer() == True)
...
# ─────────────────────────────────────────────
# 4. ArgumentTransformationSpecialist
# ─────────────────────────────────────────────
...
test("question préservée après deepcopy",
     newProb.getQuestion().getArgument().getIndex() == 1)


# ─────────────────────────────────────────────
# 5. AttackTransformationSpecialist
# ─────────────────────────────────────────────
...
test("attaque ajoutée : bonne direction (1→3)",
...
# ─────────────────────────────────────────────
# 6. TransformAndIsomorphStrategy — passe 1
# ─────────────────────────────────────────────
print("\n── TransformAndIsomorphStrategy — passe 1 (iso direct) ──")
...
test("passe 1 : réponse correcte",             sol_direct is not None and sol_direct.getAnswer() == True)


# ─────────────────────────────────────────────
# 7. TransformAndIsomorphStrategy — passe 2
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# 9. Sémantiques (Admissible, Complete, Stable)
# ─────────────────────────────────────────────
...
test("Admissible : {1, 3} est une extension valide", sem_admissible.isExtension(af_sem, ext_ok))

# {1, 2} contient un conflit (1->2) -> Non admissible
ext_conflict = Extension({Argument(1), Argument(2)})
test("Admissible : {1, 2} rejeté pour conflit", not sem_admissible.isExtension(af_sem, ext_conflict))

# {3} n'est pas défendu contre 2 -> Non admissible
ext_undefended = Extension({Argument(3)})
test("Admissible : {3} rejeté car non défendu", not sem_admissible.isExtension(af_sem, ext_undefended))
...
test("Complete : {1, 3} est une extension complète", sem_complete.isExtension(af_sem, ext_ok))

# {} (l'extension vide) défend l'argument 1. Comme 1 n'est pas dedans, {} n'est PAS complète
ext_vide = Extension(set())
test("Complete : Extension vide rejetée si elle omet un argument défendu (1)", not sem_complete.isExtension(af_sem, ext_vide))
...
test("Stable : {1, 3} rejeté car il n'attaque pas tous les arguments extérieurs (4)", not sem_stable.isExtension(af_sem, ext_ok))
...
test("Stable : {1, 3} est stable (attaque tout le monde à l'extérieur)", sem_stable.isExtension(af_stable_valid, ext_stable))
